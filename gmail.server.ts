...
// Erro temporário (limite de uso ou créditos de IA esgotados): não é culpa
// daquele e-mail específico, então quem chama não deve gravá-lo como "erro"
// permanente — o certo é parar e tentar de novo mais tarde.
export class GmailSyncPausedError extends Error {}
...
export async function parseQuoteWithAI(input: {
  ...
  if (!res.ok) {
    ...
    if (res.status === 429)
      throw new GmailSyncPausedError("Limite de leituras automáticas atingido. Tente de novo em alguns minutos.");
    if (res.status === 402)
      throw new GmailSyncPausedError("Créditos de IA insuficientes para ler os e-mails.");
